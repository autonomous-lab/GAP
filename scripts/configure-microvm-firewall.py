...
def configure(config, script, apply=False, container=None, no_provider_script=False):
    ...
    path=None if no_provider_script else Path(script)
    if path and path.is_symlink():raise ValueError('Refusing a symlink firewall script')
    source=path.read_text() if path else MARKER
    ...
    print('Rules applied'+('; provider persistence: '+str(path) if path else '; use the systemd timer for persistence'))
...
if __name__=='__main__':
    ...
    p.add_argument('--no-provider-script',action='store_true',help='Use systemd persistence on hosts without the Elestio firewall script')
    p.add_argument('--container',help='Bridge-network edge container for scalable range DNAT')
    a=p.parse_args();configure(a.config,a.script,a.apply,a.container,a.no_provider_script)
